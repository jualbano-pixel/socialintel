...
const ENRICHMENT_ENDPOINTS = {
  ...
  trendingLinks: 'trending-links',
  mostActiveSites: 'most-active-sites',
...
export function enrichmentSections() {
  return Object.keys(ENRICHMENT_ENDPOINTS);
}

export async function getProjectEnrichment(projectId, dateFrom, dateTo, sections = enrichmentSections()) {
  if (!projectId) throw new Brand24RestError('Monitor was not found.');
  const selectedSections = (Array.isArray(sections) && sections.length ? sections : enrichmentSections())
    .filter(section => ENRICHMENT_ENDPOINTS[section]);
  const settled = await Promise.allSettled(selectedSections.map(async section => {
    const data = await request(`/project/${projectId}/${ENRICHMENT_ENDPOINTS[section]}`, {
      params: {
        date_from: dateFrom,
        date_to: dateTo,
        from: dateFrom,
      ...
      label: `${section} for project ${projectId}`,
    });
    return [section, data?.data ?? data?.message ?? data];
  }));
  return settled.reduce((payload, result, index) => {
    const section = selectedSections[index];
    if (result.status === 'fulfilled') {
      const [key, value] = result.value;
      payload[key] = value;
    } else {
      payload.errors[section] = result.reason instanceof Brand24RestError
        ? result.reason.message
        : 'Tracking enrichment request failed.';
    }
    return payload;
  }, {
    projectId,
