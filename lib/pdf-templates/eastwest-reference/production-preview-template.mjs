...
function addMonitoringEventsPage({ pages, contract, period, sourceRunId }) {
  ...
  pages.push(page({
    ...
    body: `<div class="approval-banner">${statusTag(record)} Agent-produced context remains subject to editorial verification. Fields are shown only when supplied.</div><div class="event-list">${eventCards}</div><div class="conflict-callout">This source statement conflicts with the live tracking total shown elsewhere in the same completed run. Both are retained for review.</div>`,
  }));
}

function addCompetitorNotesPages({ pages, contract, period, sourceRunId }) {
...
function addFindingsPages({ pages, contract, period, sourceRunId }) {
  ...
  if ((!positive?.items?.length) && (!negative?.items?.length) && !executive?.text) return false;
  pages.push(page({
    title: 'REPORT SUMMARY', period, sourceRunId, className: 'report-summary',
    body: `<div class="approval-banner">${statusTag(executive)} Agent-produced narrative - editorial approval required before client export.</div>${executive?.text ? `<div class="executive-copy">${esc(executive.text)}</div>` : missing('Executive summary unavailable', 'No current-period summary was supplied.')}`,
  }));
  const positiveItems = positive?.items || [];
  const negativeItems = negative?.items || [];
  const pageCount = Math.max(Math.ceil(positiveItems.length / 2), Math.ceil(negativeItems.length / 1), 1);
...
export function eastWestProductionPreviewHtml({ contract, inventory, checklist, includeDiagnostics = true }) {
  ...
  addConversationDriverPages({ pages, contract, period, sourceRunId });
  addMonitoringEventsPage({ pages, contract, period, sourceRunId });
  ...
  addCompetitorNotesPages({ pages, contract, period, sourceRunId });
  ...
  pages.push(page({ title: 'SHARE OF VOICE - MENTIONS', period, sourceRunId, className: `sov-page${isMeasurementManifest ? ' measurement-page' : ''}`, body: isMeasurementManifest
    ...
    : sovRows.length ? `<div class="sov-bars">${sovRows.map(row => `<div class="sov-row ${row.state !== 'verified' ? 'needs-review' : ''}"><span>${esc(row.name)}</span><i style="width:${row.sovPercentage?.value ?? 1}%"></i><b>${row.state === 'verified' ? `${row.sovPercentage.value}% - ${num(row.mentions.value)}` : 'Unavailable - monitored zero under review'}</b></div>`).join('')}</div><div class="conflict-callout">Current-period SOV is rendered from the completed export only. EastWest SOV mentions (790) differ from the report total (733); the PDF layer does not reconcile these snapshots.</div>` : missing('Current-period SOV unavailable', 'The existing report output did not include a completed compatible competitor cohort.') }));
  ...
  if (!addFindingsPages({ pages, contract, period, sourceRunId })) pages.push(page({ title: 'REPORT SUMMARY', period, sourceRunId, body: missing('Approved summary unavailable', 'No approved current-period findings were included in the available output.') }));
  if (!addRecommendationsPages({ pages, contract, period, sourceRunId })) pages.push(page({ title: 'RECOMMENDATIONS', period, sourceRunId, body: missing('Approved recommendations unavailable', 'Recommendations must be supplied and approved for this reporting period.') }));
