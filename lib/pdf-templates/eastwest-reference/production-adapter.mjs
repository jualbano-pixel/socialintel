...
function narrativeEligibility(text, { authority, sovUnavailable, checkMeasurementClaims = true, qualityFlagIds }) {
  const reasonCodes = [];
  const linkedFlags = [];
  ...
  if (checkMeasurementClaims && deterministicMeasurementConflicts(text, authority).length) {
...
function competitorAuthority(contract, name) {
  ...
  if (!row || row.state !== status.verified) return {};
  return {
...
function applyDeterministicPresentationQuarantine(adapted, manifest, intelligenceSource) {
  ...
  const itemNarrative = (record, authorityForItem = () => authority, checkMeasurementClaims = true) => withItemEligibility(
    record,
  ...
  narratives.positiveThemes = itemNarrative(narratives.positiveThemes, undefined, false);
  narratives.negativeThemes = itemNarrative(narratives.negativeThemes, undefined, false);
  narratives.recommendations = itemNarrative(narratives.recommendations);
